	...
	if (pPlayer->IsSpectator())
	{
		...
		for (int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if (pPlayerLoop && pPlayerLoop->IsNetClient())
			{
				...
				if ((Team == Type || Close == Type) && !pPlayerLoop->IsSpectator() && pPlayer != pPlayerLoop)
					continue;
				...
				if (ag_match_running.value != 0.0f && ag_match_mute.value == 2.0f && !pPlayer->IsAdmin() && !pPlayerLoop->IsSpectator() && pPlayer != pPlayerLoop)
					continue;
		...
		for (int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if (pPlayerLoop && pPlayerLoop->IsNetClient())
			{
				...
				if (Close == Type && pPlayer != pPlayerLoop)
				{
					...
					Vector vDist = pPlayerLoop->pev->origin - pPlayer->pev->origin;
					if (vDist.Length() > 700)
						continue;
				}
				...
				if (ag_match_running.value != 0.0f && ag_match_mute.value == 2.0f && !pPlayer->IsTeammate(pPlayerLoop) && !pPlayerLoop->IsSpectator())
					continue;
