...
Schedule_t slTlkIdleWatchClient[] = {
	{ tlTlkIdleWatchClient,
	    ...
	    bits_COND_NEW_ENEMY | bits_COND_LIGHT_DAMAGE | bits_COND_HEAVY_DAMAGE | bits_COND_HEAR_SOUND | bits_COND_SMELL | bits_COND_CLIENT_PUSH | bits_COND_CLIENT_UNSEEN | bits_COND_PROVOKED,
...
void CTalkMonster ::StartTask(Task_t *pTask)
{
	...
	case TASK_TLK_IDEALYAW:
		if (m_hTalkTarget != NULL)
		{
			...
			if (yaw < 0)
			{
				pev->ideal_yaw = min(yaw + 45, 0.f) + pev->angles.y;
			}
			else
			{
				pev->ideal_yaw = max(yaw - 45, 0.f) + pev->angles.y;
			}
		}
		TaskComplete();
		break;
	...
	if (pTarget != NULL)
	{
		if (pTarget->IsPlayer())
		{
			if (pTarget->IsAlive())
			{
				...
				else if (!FBitSet(m_bitsSaid, bit_saidDamageMedium) && (m_hTargetEnt->pev->health <= m_hTargetEnt->pev->max_health / 4))
				{
				...
				else if (!FBitSet(m_bitsSaid, bit_saidDamageLight) && (m_hTargetEnt->pev->health <= m_hTargetEnt->pev->max_health / 2))
				{
	...
	case SCHED_IDLE_STAND:
	{
		...
		if (!IsTalking() && HasConditions(bits_COND_SEE_CLIENT) && RANDOM_LONG(0, 6) == 0)
		{
			...
			if (pPlayer)
			{
				// watch the client.
				UTIL_MakeVectors(pPlayer->v.angles);
				if ((pPlayer->v.origin - pev->origin).Length2D() < TLK_STARE_DIST && UTIL_DotPoints(pPlayer->v.origin, pev->origin, gpGlobals->v_forward) >= m_flFieldOfView)
				{
