...
void CBaseMonster ::MaintainSchedule(void)
{
	...
	for (i = 0; i < 10; i++)
	{
		...
		if (!FScheduleValid() || m_MonsterState != m_IdealMonsterState)
		{
			...
			if (m_IdealMonsterState != MONSTERSTATE_DEAD && (m_IdealMonsterState != MONSTERSTATE_SCRIPT || m_IdealMonsterState == m_MonsterState))
			{
				if ((m_afConditions && !HasConditions(bits_COND_SCHEDULE_DONE)) || (m_pSchedule && (m_pSchedule->iInterruptMask & bits_COND_SCHEDULE_DONE)) || ((m_MonsterState == MONSTERSTATE_COMBAT) && (m_hEnemy == NULL)))
				{
					GetIdealState();
				}
			}
...
void CBaseMonster ::StartTask(Task_t *pTask)
{
	...
	case TASK_STRAFE_PATH:
	{
		...
		vec2DirToPoint = (m_Route[0].vecLocation - pev->origin).AsVector2D();
		vec2DirToPoint.NormalizeInPlace();
		vec2RightSide = gpGlobals->v_right.AsVector2D();
		vec2RightSide.NormalizeInPlace();
