...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	auto minRespawnTime = min(ag_min_respawn_time.value, 120.0f);
	if (CVAR_GET_FLOAT("mp_forcerespawn") > 0.0f)
		minRespawnTime = min(minRespawnTime, ag_forcerespawn_time.value);
	...
	if (!(pev->button & IN_ATTACK || pev->button & IN_USE) && !(g_pGameRules->IsMultiplayer() && CVAR_GET_FLOAT("mp_forcerespawn") > 0 && (gpGlobals->time > (m_flLastUnaliveTime + ag_forcerespawn_time.value))))
		return;
	//-- Martin Webrant
...
edict_t *EntSelectSpawnPoint(CBasePlayer *pPlayer)
{
	...
	else if (g_pGameRules->IsDeathmatch())
	{
		...
		{
		case 0:
			pSpot = spawnChooser.GetClassicSpawnPoint();
			break;
		case 1:
			pSpot = spawnChooser.GetRandomSpawnPoint();
			break;
		case 2:
			pSpot = spawnChooser.GetFarSpawnPoint();
			break;
		case 3:
			pSpot = spawnChooser.GetPositionAwareSpawnPoint();
			break;
		case 4:
			pSpot = spawnChooser.GetHL25SpawnPoint();
			break;
		default:
			pSpot = spawnChooser.GetClassicSpawnPoint();
			break;
...
void CBasePlayer::MoveToInfoIntermission(edict_t *pSpot)
{
	if (!FNullEnt(pSpot))
	{
		...
		if (!FNullEnt(pTarget))
		{
			...
			pev->v_angle = UTIL_VecToAngles((VARS(pTarget)->origin - VARS(pSpot)->origin).Normalized());
			pev->v_angle.x = -pev->v_angle.x;
			pev->v_angle.z = 0;
			pev->angles = pev->v_angle;
			pev->fixangle = true;
...
void CBasePlayer::UpdateFlagStatus(CBasePlayer *pPlayer)
{
	...
	if (m_iFlagStatus1Last == Carry && (iFlagStatus1 != Carry) || m_iFlagStatus2Last == Carry && (iFlagStatus2 != Carry))
	{
...
void CBasePlayer::Init()
{
	...
	m_flLastNameChange = AgTime() - max(ag_flood_name_spec_cooldown.value, ag_flood_name_cooldown.value) - 0.001f;
	m_flLastModelChange = AgTime() - max(ag_flood_model_spec_cooldown.value, ag_flood_model_cooldown.value) - 0.001f;
...
{
	const auto amountToMove = (pev->origin - towardsPoint).Normalized();
