...
void RadiusDamage(Vector vecSrc, entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, float flRadius, int iClassIgnore, int bitsDamageType, CBaseEntity* directHit)
{
	...
	while ((pEntity = UTIL_FindEntityInSphere(pEntity, vecSrc, flRadius)) != NULL)
	{
		if (pEntity->pev->takedamage != DAMAGE_NO)
		{
			...
			if (tr.flFraction != 1.0 && tr.pHit != pEntity->edict() && directHit && tr.pHit == directHit->edict() && ag_explosion_fix.value != 0.0f)
			{
			...
			{ // the explosion can 'see' this entity, so hurt them!
				if (tr.fStartSolid)
				{
					...
					if (!UTIL_IsPointInEntity(vecSrc, pEntity))
					{
						...
						if (ag_gauss_fix.value >= 2.0f && FClassnameIs(pevInflictor, "weapon_gauss") && pEntity == CBaseEntity::Instance(pevAttacker))
						{
							continue;
						}
					}
