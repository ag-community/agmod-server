...
FILE_GLOBAL COMMANDS s_Commands[] =
	{
		...
		spectator,
		spectator_client,
		"agforcespectator <name/#number> Force a player into specmode",
		"agforceteamup",
		teamup,
		teamup_client,
		"agforceteamup <name/#number> team - Force a player into a team",
		"agkick",
		NULL,
		kick_client,
		...
		start,
		start_client,
		"agstart <full/nolock> - Start a match.",
		"agabort",
		agabort,
		agabort_client,
		...
		variables,
		variables_client,
		"variables - Server variable list.",

		"agmaxtime",
		agmaxtime,
		agmaxtime_client,
		"agmaxtime - Set the time limit to the max allowed by the server",
		"agmoretime",
		agmoretime,
		agmoretime_client,
		"agmoretime - Extend the time limit, ignoring the max limit",

		"agdefault",
		agdefault,
		agdefault_client,
		"agdefault - Set gamemode-related cvars to default values",
};
...
void AgCommand::Help(CBasePlayer* pPlayer)
{
	...
	for (int i = 0; i < sizeof(s_Commands) / sizeof(s_Commands[0]); i++)
	{
		...
		else if (pPlayer && 0 == strcmp(s_Commands[i].szCommand, "agaddadmin") && 0 == strcmp(s_Commands[i].szCommand, "aglistadmins") && 0 == strcmp(s_Commands[i].szCommand, "agdeladmin"))
			AgConsole(s_Commands[i].szDescription, pPlayer);
	}
